...
import os
from paintcanvas.dialog import OpacityDialog, RenameDialog
from paintcanvas.geometry import grow_rect
...
class LayerStackView(QtWidgets.QWidget):
    ...
    @property
    def layerstack(self):
        return self.canvas.model.layerstack
    ...
    def add_layers_from_paths(self, paths):
        ...
        for image in images:
            ...
            self.canvas.model.add_layer(undo=False, name="Imported image")
            self.canvas.model.add_shape(Bitmap(image, rect))
    ...
    def drop(self):
        ...
        if action == 'before' and index - 1 == self.handle_index:
            return
        self.canvas.model.move_layer(self.handle_index, index)
