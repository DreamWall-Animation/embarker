...
from embarker.audio import AudioPlayer
...
# Before playback, we cache first frames of a number of following videos:
FIRST_FRAMES_CACHE_COUNT = 16
...
class MediaPlayer(QOpenGLWidget):
    ...
    def __init__(self, viewportmapper: NDCViewportMapper, playlist: Playlist):
        ...
        self.playlist.playlist_modified.connect(self.load_audio)
        self.playlist.playlist_modified.connect(self.play_current_frame)
    ...
    def play(self, emit_signal=True):
        if not self.playlist._containers:
            return
        if emit_signal:
        ...
        # Cache first frames
        frames_to_cache = [
            f for f in self.playlist.first_frames.values()
            if f >= frame]
        for f in frames_to_cache[:FIRST_FRAMES_CACHE_COUNT]:
    ...
    def playback_next_frame(self):
        ...
        frame = self.playlist.frame
        overrides = frame in self.playlist.frames_images_overrides
        if overrides and not self.playlist.mute_annotations:
            image = self.playlist.frames_images_overrides[frame]
        else:
            try:
                image = self.playlist.frames_images[frame]
            except KeyError:  # 1/10_000 sec of delay
                self.pause()
                return
        ...
        if last == frame:
            ...
            if self.playlist.playback_loop:
                ...
                QtCore.QTimer.singleShot(interval, partial(self.play, False))
            return
    ...
    def draw_image(self, image, frame):
        ...
        if size != self.image_size:
            ...
            gl.glTexImage2D(*glTexImage2D_args)
            gl.glTexSubImage2D(*glTexSubImage2D_args)
        else:
            try:
                gl.glTexSubImage2D(*glTexSubImage2D_args)
            except GLError:
        ...
        # Draw
        self.vpm.image_size = QtCore.QSize(*self.image_size)
        self.update()

    def load_audio(self):
        self.audio_player.load_audio(self.playlist.audio_samples)

    def set_audio_output_device(self, device_index):
        if self.audio_player.set_output_device(device_index):
            # samplerate changed
            for container in self.playlist._containers:
                container.load_audio()

    def set_volume(self, value):
        self.playlist.set_volume(value)
        self.load_audio()
    ...
    def set_audio_time(self):
        if not self.playlist.frames_times:
            return
