...
class Playlist(QtCore.QObject):
    ...
    def __init__(
        ...
        self._audio_samples_source: np.array = np.array([])
    ...
    def _build_audio(self):
        ...
        if not self.containers:
            self._audio_samples_source = np.array([])
            return self.playlist_modified.emit()
        self._audio_samples_source = np.concatenate(
            [c.audio_samples for c in self._containers], axis=0)
        self._update_audio_samples()

    def _update_audio_samples(self):
        volume = self.get_volume()
        if volume == 0:
            self.audio_samples = np.zeros(
                self._audio_samples_source.shape, dtype=np.float32)
            return
        self.audio_samples = self._audio_samples_source.copy()
        if volume != 1:
            gain = volume_to_gain(volume)
            self.audio_samples *= gain

    def get_volume(self):
        return self.volume if not self.muted else 0
